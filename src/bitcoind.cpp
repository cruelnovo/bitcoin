    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                return InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see vertcoind -h for a list of options.\n", argv[i])));
            }
        }
        ...
        // -server defaults to true for vertcoind but not for the GUI so do this here
...
int main(int argc, char* argv[])
{
    ...
    // Connect vertcoind signal handlers
    noui_connect();
