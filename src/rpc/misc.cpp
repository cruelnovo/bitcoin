...
static RPCHelpMan validateaddress()
{
    return RPCHelpMan{
        ...
        {
            {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The vertcoin address to validate"},
        },
        RPCResult{
            RPCResult::Type::OBJ, "", "",
            {
                {RPCResult::Type::BOOL, "isvalid", "If the address is valid or not"},
                {RPCResult::Type::STR, "address", /*optional=*/true, "The vertcoin address validated"},
    ...
    return RPCHelpMan{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
            ...
            if (interfaces::Ipc* ipc = local_init.ipc()) {
                // Spawn a new vertcoin-node process and call makeEcho to get a
                // client pointer to a interfaces::Echo instance running in
                // that process. This is just for testing. A slightly more
                // realistic test spawning a different executable instead of
                // the same executable would add a new vertcoin-echo executable,
                // and spawn vertcoin-echo below instead of vertcoin-node. But
                // using vertcoin-node avoids the need to build and install a
                // new executable just for this one test.
                auto init = ipc->spawnProcess("vertcoin-node");
                echo = init->makeEcho();
                ipc->addCleanup(*echo, [init = init.release()] { delete init; });
            } else {
                // IPC support is not available because this is a vertcoind
                // process not a vertcoind-node process, so just create a local
